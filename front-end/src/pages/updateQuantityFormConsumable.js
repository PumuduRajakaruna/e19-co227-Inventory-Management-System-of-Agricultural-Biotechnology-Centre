...
const UpdateQuantityForm = ({ conId, onClose, onUpdateQuantity }) => {
  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:8080/consumable/getConsumableById/${conId}`)
      .then((response) => {
  ...
  //       axios.post('http://localhost:8080/chemical/labs/addChemical', newConsumableData)
  //         .then((response) => {
  ...
  const handleSubmit = (e) => {
    ...
    // Make the PUT request to update the quantity
    axios.put(`http://localhost:8080/consumable/updateQuantity/${conId}`, updatedData)
      .then((response) => {
        // Log the response for debugging
        console.log('Update response:', response.data);
        ...
        axios.post('http://localhost:8080/consumable/labs/addConsumable', newConsumableData)
          .then((response) => {
            console.log('Consumable added to the lab:', response.data);
            ...
            if (parseInt(existingData.thresholdValue) > parseInt(newFetchedQuantity-updateQuantity)) {
              ...
              const emailBodyContent = `Consumable Name: ${existingData.conName}
...
Place a new order to avoid inconveniences
            `;
              ...
              formData.append('subject', 'Alart! Consumable Ran Out of Stock');
          ...
          .catch((error) => {
            console.error('Error adding consumable to the lab:', error);
          });
      })
      .catch((error) => {
        console.error('Error updating quantity:', error);
